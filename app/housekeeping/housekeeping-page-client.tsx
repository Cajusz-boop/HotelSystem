...
import { WifiOff, Wifi, Check, Trash2, AlertTriangle, Wrench, ClipboardCheck, CheckCircle2, Clock, Star, ArrowDownCircle, ArrowUpCircle, Minus as MinusIcon, ListTodo, X, Link2, Copy } from "lucide-react";
...
type ViewMode = "rooms" | "schedule" | "performance" | "cleaning";

export default function HousekeepingPageClient({ role }: { role?: string | null }) {
  const isCleaningStaffOnly = role === "HOUSEKEEPING";
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(
    isCleaningStaffOnly ? "cleaning" : "rooms"
  );
  ...
  const roomsToClean = rooms.filter((r) => r.status === "DIRTY" || r.status === "INSPECTION");
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: HOUSEKEEPING_QUERY_KEY });
      const msg = variables.status === "CLEAN" ? "Pokój oznaczony jako posprzątany" : `Pokój ustawiony na ${STATUS_OPTIONS.find((s) => s.value === variables.status)?.label ?? variables.status}`;
      toast.success(msg);
    },
    onError: (err: Error) => {
  ...
  const renderCleaningModeContent = () => (
    <div className="flex flex-col gap-6">
      <p className="text-muted-foreground">Pokoje do posprzątania. Kliknij &quot;Posprątane&quot;, gdy pokój jest gotowy.</p>
      {roomsToClean.length === 0 ? (
        <div className="rounded-lg border border-dashed border-green-300 bg-green-50/50 p-12 text-center">
          <CheckCircle2 className="mx-auto mb-4 h-16 w-16 text-green-600" />
          <p className="text-lg font-medium text-green-800">Wszystkie pokoje posprzątane</p>
          <p className="mt-1 text-sm text-green-700">Nie ma pokoi do sprzątania.</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {roomsToClean.map((room) => (
            <div
              key={room.id}
              className="flex flex-col items-center justify-between gap-4 rounded-xl border-2 border-amber-200 bg-amber-50/30 p-6 shadow-sm"
            >
              <span className="text-2xl font-bold">Pokój {room.number}</span>
    ...
    <div className="flex flex-col gap-6 p-6 md:p-8">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl font-semibold">
          {isCleaningStaffOnly ? "Tryb sprzątania" : "Housekeeping"}
        </h1>
      ...
      {!isCleaningStaffOnly && (
        <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-4">
          ...
          <p className="mb-3 text-sm text-blue-800">
            Przekaż ten link paniom sprzątającym. Po wejściu zobaczą listę pokoi do posprzątania. Kliknięcie „Posprzątane&quot; od razu zaznaczy pokój w Housekeeping.
          </p>
          <div className="flex flex-wrap items-center gap-2">
            <code className="flex-1 rounded bg-white px-3 py-2 text-sm font-medium text-blue-900 shadow-sm">
              {typeof window !== "undefined" ? `${window.location.origin}/sprzatanie` : "https://hotel.karczma-labedz.pl/sprzatanie"}
            </code>
            <Button
              type="button"
              variant="outline"
              size="sm"
              onClick={async () => {
                const url = typeof window !== "undefined" ? `${window.location.origin}/sprzatanie` : "https://hotel.karczma-labedz.pl/sprzatanie";
      ...
      {/* View Mode Tabs – ukryte dla roli HOUSEKEEPING */}
      {!isCleaningStaffOnly && (
      ...
      {/* Tryb sprzątania – widoczny dla HOUSEKEEPING lub gdy wybrano zakładkę */}
      {(isCleaningStaffOnly || viewMode === "cleaning") && renderCleaningModeContent()}

      {viewMode === "schedule" && !isCleaningStaffOnly && (
      ...
      {viewMode === "performance" && !isCleaningStaffOnly && (
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
      ...
      {viewMode === "rooms" && !isCleaningStaffOnly && (
      <div className="flex flex-col gap-4">
