...
import { exportJpkFa } from "@/app/actions/jpk";

const MAX_JPK_RANGE_DAYS = 366;

/**
 * GET /api/finance/jpk-fa?from=YYYY-MM-DD&to=YYYY-MM-DD
 * Zwraca plik JPK_FA (Faktury) do pobrania.
 */
export async function GET(request: NextRequest) {
  ...
  const result = await exportJpkFa(from.trim(), to.trim());
  if (!result.success) {
    return NextResponse.json(
      { error: result.error ?? "Błąd generowania JPK_FA" },
      { status: 400 }
    );
  }
