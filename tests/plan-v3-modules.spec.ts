...
test.describe("Moduły: Meldunek, Kontrahenci, Ustawienia", () => {
  ...
  test("Kontrahenci — strona się otwiera", async ({ page }) => {
    await page.goto("/kontrahenci", { waitUntil: "domcontentloaded" });
    await expect(page).toHaveURL(/kontrahenci/, { timeout: 10000 });
    await expect(page).not.toHaveURL(/\/login/);
    const bodyText = await page.locator("body").textContent();
    expect(bodyText).toMatch(/Kontrahenci|Goście|Firmy|Szukaj|Panel/i);
  });

  test("Kontrahenci — zakładka Firmy", async ({ page }) => {
    await page.goto("/kontrahenci?tab=firmy", { waitUntil: "domcontentloaded" });
    await expect(page).toHaveURL(/kontrahenci/, { timeout: 10000 });
    const bodyText = await page.locator("body").textContent();
    expect(bodyText).toMatch(/Firmy|Goście|Kontrahenci/i);
  });

  test("Ustawienia — strona główna", async ({ page }) => {
    await page.goto("/ustawienia", { waitUntil: "domcontentloaded" });
    await expect(page).toHaveURL(/ustawienia/, { timeout: 15000 });
    const bodyText = await page.locator("body").textContent();
    expect(bodyText).toMatch(/Ustawienia|Dane hotelu|Użytkownicy|Numeracja|Powrót/i);
  });

  test("Ustawienia — Dane hotelu", async ({ page }) => {
    await page.goto("/ustawienia/dane-hotelu", { waitUntil: "domcontentloaded" });
