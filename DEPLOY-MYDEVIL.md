...
Skrypt: generuje Prisma (native + freebsd14), buduje Next.js, kopiuje static, wysyła **app.js** i **.next** na serwer przez **scp**. Przy `scp` wpiszesz hasło SSH (jeśli nie masz klucza). Na końcu na serwerze (SSH) uruchom: `devil www restart hotel.karczma-labedz.pl`.
...
- Zmiany w schemacie bazy: nadal wgrywaj SQL przez PhpMyAdmin (z Twojego PC do bazy z zewnątrz nie ma połączenia – P1001)
