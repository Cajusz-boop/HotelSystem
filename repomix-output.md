...
- Only files matching these patterns are included: prisma/schema.prisma, src/app/api/**/route.ts, src/actions/**/*.ts, src/types/**/*.ts, src/lib/**/*.ts
...
model Property {
  ...
  paymentStatusColors Json?      // opcjonalna paleta kolorów płatności: { PAID: "rgb(...)", PARTIAL: "...", UNPAID: "..." }
  statusCombinationColors Json?  // macierz kolorów: { "CONFIRMED_PAID": "#22c55e", "CHECKED_IN_UNPAID": "#ef4444", ... }
  reservationStatusLabels Json?  // etykiety statusów: { CONFIRMED: "Potwierdzona", ... }
  reservationStatusDescriptions Json? // opisy statusów: { CONFIRMED: "Opis...", ... }
  ...
  dailyRateOverrides DailyRateOverride[]
...
model RoomType {
  ...
  description    String?  @db.Text  // opis typu pokoju (do booking engine)
  basePrice      Decimal? @db.Decimal(10, 2)
  sortOrder      Int      @default(0)
  visibleInStats Boolean  @default(true)  // czy uwzględniać w raportach obłożenia
  maxOccupancy   Int?     // maksymalna liczba osób
  bedsDescription String? @db.VarChar(200)  // opis łóżek (np. "1 queen + 1 sofa")
  photoUrl       String?  @db.VarChar(500)  // zdjęcie typu pokoju
  translations   Json?    // tłumaczenia nazwy/opisu: { en: { name, description }, de: { ... } }
  rateCodeId     String?  // domyślny kod stawki (np. COMFORT-WIDOK) – auto przy wyborze pokoju
...
model RatePlan {
  ...
  includedMealPlan String? // wyżywienie w cenie: RO, BB, HB, FB
  createdAt      DateTime  @default(now())
  updatedAt      DateTime  @updatedAt
  @@index([roomTypeId])
...
model Room {
  ...
  sellPriority  Int        @default(0)    // kolejność propozycji przy sprzedaży (niższy = wyższy priorytet)
...
model Guest {
  ...
  customFields  Json?    // własne pola gościa (np. { "favorite_room": "101", "diet": "bezglutenowa" })
  ...
  guestDiscounts  GuestDiscount[]

  @@index([loyaltyTierId])
}

// Rabat przypisany do gościa na określony okres
model GuestDiscount {
  id         String   @id @default(cuid())
  guestId    String
...
model Company {
  id              String   @id @default(cuid())
  nip             String   @unique  // NIP (10 cyfr) lub numer VAT UE (np. DE123456789)
  ...
  invoices      Invoice[]  // faktury zbiorcze (companyId)
...
// Ceny dzienne za pokój (możliwość edycji stawki za każdy dzień osobno)
model ReservationDayRate {
  id            String      @id @default(cuid())
  reservationId String
  reservation   Reservation @relation(fields: [reservationId], references: [id], onDelete: Cascade)
...
model RateCode {
  id             String   @id @default(cuid())
  code           String   @unique  // np. "BB", "RO", "NET", "STALY", "WESELNY"
  name           String   // np. "Śniadanie", "Gość stały"
  price          Decimal? @db.Decimal(10, 2)  // stała cena za dobę (gdy bez wzoru)
  basePrice      Decimal? @db.Decimal(10, 2)  // wzór: basePrice + pricePerPerson × pax (np. 185 + 50×osoby)
  pricePerPerson Decimal? @db.Decimal(10, 2)  // dopłata za osobę przy wzorze
  createdAt      DateTime @default(now())
  updatedAt      DateTime @updatedAt
  reservations Reservation[]
  corporateContracts CorporateContract[]
  travelAgents TravelAgent[]  // agenci korzystający z tego kodu
  roomTypes   RoomType[]  // typy pokoi z tym domyślnym kodem
}
...
model Reservation {
  ...
  isCreditCardGuaranteed Boolean @default(false) // karta gwarancyjna aktywna
  notesVisibleOnChart Boolean @default(false)    // uwagi widoczne na pasku w Tape Chart
  showNotesOnChart Boolean @default(false)       // alias / legacy
  advancePayment Json?         // przedpłata: { required, amount, dueDate, paid, paidDate, paidAmount, method, notes }
  advanceDueDate DateTime?     @db.Date  // termin wpłaty zaliczki
  extraStatus    String?       // dodatkowy status: VIP, AWAITING_PAYMENT, COMPLAINT, itp.
  reminderAt     DateTime?     // przypomnienie do rezerwacji
  externalReservationNumber String?  // nr rezerwacji w OTA/Booking
  currency       String?       // waluta (PLN, EUR, USD)
  cancellationReason String?   // powód anulacji rezerwacji
  deletionReason String?       // powód usunięcia (audit)
  ...
  /** Gdy true: faktura pokazuje jedną linię „Usługa hotelowa” z całą sumą (nocleg + gastronomia + inne) */
  invoiceSingleLine Boolean   @default(true)
  /** Zakres faktury: ALL (wszystkie), HOTEL_ONLY (tylko hotel), GASTRONOMY_ONLY (tylko gastronomia) */
  invoiceScope     String    @default("ALL")
  /** Nadpisana kwota wpłat (widoczna na fakturze) – gdy null, używana suma z transakcji */
  paidAmountOverride Decimal? @db.Decimal(12, 2)
  createdAt   DateTime         @default(now())
  updatedAt   DateTime         @updatedAt
  ...
  reservationFolios ReservationFolio[]
  reservationDayRates ReservationDayRate[]
...
// ═══ SŁOWNIK DAŃ (centralna baza dań do pakietów menu) ═══
model Dish {
  id           String   @id @default(cuid())
  name         String   // unikalna nazwa dania
  code         String?  @unique // opcjonalny kod (np. do synchronizacji)
  defaultPrice Decimal  @db.Decimal(10, 2) // cena / koszt za porcję
  vatRate      Decimal  @db.Decimal(5, 4)  // np. 0.08 dla 8%
  category     String?  // np. "Zupa", "Danie główne", "Deser"
  gtuCode      String?  // kod GTU
  allergens    Json?    // ["gluten","mleko",...]
...
// ═══ PAKIETY MENU IMPREZOWEGO ═══
// Osobne od model Package (który dotyczy rezerwacji hotelowych/SPA)
model MenuPackage {
  id          String   @id @default(cuid())
  code        String   @unique           // "stypa_58", "wesele_290" — zachowaj stare ID
  name        String                      // "Stypa podstawowa", "Wesele propozycja 1"
  price       Decimal  @db.Decimal(10,2)  // cena za osobę (58, 290, itd.)
  eventTypes  Json                        // ["STYPA"] lub ["URODZINY","CHRZCINY","KOMUNIA"]
  isActive    Boolean  @default(true)
  sortOrder   Int      @default(0)

  sections    MenuPackageSection[]
  surcharges  MenuPackageSurcharge[]
  rules       Json?                       // regulamin — string[]

  createdAt   DateTime @default(now())
  updatedAt   DateTime @updatedAt
}

model MenuPackageSection {
  id          String   @id @default(cuid())
  packageId   String
  package     MenuPackage @relation(fields: [packageId], references: [id], onDelete: Cascade)

  code        String                      // "zupa", "glowne", "surowki"
  label       String                      // "Zupa", "Dania główne"
  type        String                      // "fixed" lub "wybor"
  choiceLimit Int?                         // limit wyboru (null dla fixed)
  dishes      Json                        // string[] — nazwy dań (cache / legacy)
  dishIds     Json?                       // string[] — ID z Dish (słownik); gdy obecne, ma pierwszeństwo
  sortOrder   Int      @default(0)

  @@index([packageId])
}

model MenuPackageSurcharge {
  id          String   @id @default(cuid())
  packageId   String
  package     MenuPackage @relation(fields: [packageId], references: [id], onDelete: Cascade)

  code        String                      // "napoje", "ciasta", "udko"
  label       String                      // "Napoje nielimitowane", "Ciasta"
  pricePerPerson Decimal? @db.Decimal(10,2) // 25 zł/os (null jeśli ryczałt)
  flatPrice   Decimal? @db.Decimal(10,2)   // 800 zł ryczałt (null jeśli per person)
  hasChoice   Boolean  @default(false)     // czy ma opcje do wyboru
  choiceLimit Int?                          // ile opcji wybrać
  options     Json?                         // string[] — opcje do wyboru
...
model Transaction {
  ...
  gtuCode        String?  // kod GTU (JPK) – np. GTU_11 dla noclegu
  ...
  // Nadpisanie ceny noclegu przez recepcjonistę (nie nadpisuj ceną z cennika)
  isManualOverride  Boolean   @default(false)
  originalAmount    Decimal?  @db.Decimal(12, 2) // oryginalna kwota przed nadpisaniem (do audytu)
  ...
  externalRef    String?  @db.Text  // np. numer zamówienia z restauracji (JSON z pozycjami)
...
model Proforma {
  ...
  createdAt      DateTime @default(now())
  /** Nadpisania pól edytowalnych: buyer*, placeOfIssue, deliveryDate, payment*, notes, footerText, thanksText, sellerOverride, lineItems */
  overrides      Json?
...
model Invoice {
  id             String   @id @default(cuid())
  reservationId  String?  // opcjonalne: null dla faktur na produkty (stypy, vouchery) i zbiorczych
  reservation    Reservation? @relation(fields: [reservationId], references: [id], onDelete: Cascade)
  companyId      String?  // dla faktury zbiorczej – wiele rezerwacji jednej firmy
  company        Company? @relation(fields: [companyId], references: [id], onDelete: Restrict)
  sourceType     String   @default("RESERVATION")  // RESERVATION, EVENT, VOUCHER, MANUAL, CONSOLIDATED
  sourceId       String?  // ID EventOrder, GiftVoucher itp. (gdy sourceType != RESERVATION)
  invoiceType    String?  @default("NORMAL")  // NORMAL, ADVANCE, FINAL
  advanceInvoiceId String?  // gdy invoiceType = FINAL: ID faktury zaliczkowej rozliczanej
  ...
  buyerNip      String?  // opcjonalne – dla faktury imiennej (osoba fizyczna bez NIP)
  buyerName     String
  buyerAddress  String?
  buyerPostalCode String?
  buyerCity      String?
  
  // Odbiorca (gdy inny niż nabywca, np. szkoła/oddział)
  receiverName     String?
  ...
  // Daty i miejsce
  issuedAt      DateTime @default(now())
  deliveryDate  DateTime?  // Data dostawy/wykonania usługi
  placeOfIssue  String?    // Miejsce wystawienia
  
  // Płatność
  paymentMethod   String?    // Forma płatności: "przelew", "gotówka", "karta"
  paymentDueDate  DateTime?  // Termin płatności
  paymentDays     Int?       // Ilość dni na płatność (alternatywa dla paymentDueDate)
  
  // Podpisy
  issuedByName    String?    // Osoba upoważniona do wystawienia
  receivedByName  String?    // Osoba upoważniona do odbioru
  ...
  paymentBreakdown Json?    // rozbicie płatności: [{ type: "CASH", amount: 100 }, { type: "CARD", amount: 50 }]
  customFieldValues Json?   // własne pola faktury (np. zamówienie, projekt)
  notes         String?  @db.Text  // uwagi widoczne na fakturze
  /** Nadpisanie danych sprzedawcy na tej fakturze (sellerName, sellerAddress, ...) */
  sellerOverride Json?
  /** Nadpisanie stopki: footerText, thanksText */
  documentOverrides Json?
  /** Zakres użyty przy tworzeniu: ALL, HOTEL_ONLY, GASTRONOMY_ONLY (dla faktur z rezerwacji) */
  invoiceScope  String?  // ALL | HOTEL_ONLY | GASTRONOMY_ONLY

  /** Dla faktury zbiorczej: ISSUED, PAID, CANCELLED. Null dla zwykłej faktury. */
  consolidatedStatus String?
  paidAt         DateTime?

  /** Okres rozliczeniowy – tylko dla faktury zbiorczej */
  periodFrom     DateTime? @db.Date
  periodTo       DateTime? @db.Date
...
// Powiązanie faktury zbiorczej z rezerwacjami (snapshot danych)
model InvoiceReservation {
  ...
  @@unique([invoiceId, reservationId])
  @@index([invoiceId])
  @@index([reservationId])
}

// Pozycje faktury (dla faktur na produkty: stypy, vouchery, usługi – bez rezerwacji)
model InvoiceLineItem {
  id          String   @id @default(cuid())
  invoiceId   String
  invoice     Invoice  @relation(fields: [invoiceId], references: [id], onDelete: Cascade)
  description String   @db.Text  // np. "Stypa – 50 osób", "Voucher 500 PLN", "Wynajem sali"
  quantity    Decimal  @db.Decimal(10, 2) @default(1)
  unit        String   @default("szt.")  // szt., os., godz., usługa
  unitPrice   Decimal  @db.Decimal(12, 2)  // cena jednostkowa netto
  vatRate     Decimal  @db.Decimal(5, 2) @default(8)  // stawka VAT (%)
  amountNet   Decimal  @db.Decimal(12, 2)
  amountVat   Decimal  @db.Decimal(12, 2)
  amountGross Decimal  @db.Decimal(12, 2)
...
// Nadpisania cen dziennych dla typu pokoju
...
model ServiceRate {
  ...
  code              String
  price             Decimal  @db.Decimal(10, 2)
  calculationMethod String   @default("PER_NIGHT")
...
model HotelConfig {
  ...
  sessionSettings Json?   // { screenLockMinutes: number, hardLogoutMinutes: number } — 0 = wyłączony lock
  maxVoidHours   Int?     // maks. czas na anulowanie transakcji (w godzinach); null = bez limitu
  bankAccount    String?  @db.VarChar(50)  // numer konta bankowego
  bankName       String?  @db.VarChar(200)  // nazwa banku
  bookingNotificationEmail String?  @db.VarChar(254)  // e-mail do powiadomień o rezerwacjach
  eventTypeFieldsConfig    Json?    // konfiguracja widoczności pól dla typów imprez: { WESELE: { showChurch: true, ... }, ... }
  updatedAt      DateTime @updatedAt
}
...
model GroupQuote {
  ...
  totalAmount   Decimal?  @db.Decimal(12, 2)
  items         Json?     // pozycje: [{ name, unit, quantity, unitPriceNet, vatRate, netAmount, vatAmount, grossAmount }]
  // Dane sprzedawcy / nagłówek
  clientName    String?
  clientNip     String?
...
// MICE – zlecenia realizacji (imprezy, bankiety, wesela) – rozbudowany formularz imprezy
model EventOrder {
  id          String   @id @default(cuid())
  name        String   // np. "Wesele Kowalski – 15.06.2026"

  // Dane podstawowe
  eventType   String   @default("INNE")  // WESELE | KOMUNIA | CHRZCINY | URODZINY | STYPA | FIRMOWA | INNE
  clientName   String?
  clientPhone  String?
  ...
  roomName    String?   // Sala Duża | Sala Mała | Ogród

  // Goście
  guestCount        Int?
  adultsCount       Int?
  children03        Int?   // dzieci 0-3 lat
  children47        Int?   // dzieci 4-7 lat
  orchestraCount    Int?
  cameramanCount    Int?
  photographerCount Int?

  // Wesele – pola specyficzne
  churchTime       String?  // HH:mm
  brideGroomTable  String?
  orchestraTable   String?

  // Menu
  packageId        String?  // ID pakietu menu (opcjonalne)
  cakesAndDesserts String?  @db.Text
  menu             Json?    // Zapisane menu (pakiet + wybory + dopłaty)

  // Tort
  cakeOrderedAt   String?  // gdzie zamówiony
  cakeArrivalTime String?  // HH:mm
  cakeServedAt    String?  // HH:mm

  // Napoje i alkohol
  drinksArrival      String?
  drinksStorage      String?
  champagneStorage   String?
  alcoholUnderStairs Boolean @default(false)
  firstBottlesBy     String?
  coolersWithIce     String?
  alcoholServiceBy   String?
  wineLocation       String?
  beerWhen           String?
  alcoholAtTeamTable Boolean @default(false)

  // Dekoracje i układ
  cakesSwedishTable  Boolean @default(false)
  fruitsSwedishTable Boolean @default(false)
  ownFlowers         Boolean @default(false)
  ownVases           Boolean @default(false)
  decorationColor    String?
  placeCards         Boolean @default(false)
  placeCardsLayout   String?
  tableLayout        String?
  breadWelcomeBy     String?

  // Dodatkowe
  extraAttractions  String?  @db.Text
  specialRequests   String?  @db.Text
  facebookConsent   Boolean  @default(false)
  ownNapkins        Boolean  @default(false)
  dutyPerson        String?

  // Afterparty (WESELE i URODZINY)
  afterpartyEnabled  Boolean @default(false)
  afterpartyTimeFrom String?
  afterpartyTimeTo   String?
  afterpartyGuests   Int?
  afterpartyMenu     String?  @db.Text
  afterpartyMusic    String?

  // Google integracje (multi-room: googleCalendarEvents = [{roomName, eventId, calId}])
  googleCalendarEventId   String?
  googleCalendarCalId     String?   @db.VarChar(255)
  googleCalendarEvents    Json?
  googleCalendarSynced    Boolean   @default(false)
  googleCalendarSyncedAt  DateTime?
  googleCalendarError     String?   @db.Text
  googleCalendarUpdatedAt DateTime?
  googleAttachments       Json?
  ...
  // Poprawiny (dzień po weselu)
  isPoprawiny   Boolean @default(false)
  parentEventId String?

  // Status i meta (zachowane z oryginalnego modelu)
  eventNumber String?  @db.VarChar(20)
  quoteId     String?
  roomIds   Json?
  dateFrom  DateTime @db.Date
  dateTo    DateTime @db.Date
  ...
  checklist  Json?     // { "cake": true, "flowers": false, ... }
  createdAt DateTime @default(now())
  updatedAt DateTime @updatedAt
...
// Kanały watch Google Calendar (webhooks)
...
model MenuItem {
  id         String   @id @default(cuid())
  externalId String?  @unique // np. AssortmentID z POS-Karczma – do synchronizacji
...
// Baza asortymentu – pozycje do fakturowania (nazwa, cena, VAT, GTU)
model AssortmentItem {
  id           String   @id @default(cuid())
  name         String
  defaultPrice Decimal  @db.Decimal(10, 2)
  vatRate      Decimal  @db.Decimal(5, 4) // np. 0.23 dla 23%
  gtuCode      String?
  category     String?
  sortOrder    Int      @default(0)
  isActive     Boolean  @default(true)
...
model CashDocument {
  id            String       @id @default(cuid())
  type          String       // KP (kasa przyjmie) / KW (kasa wyda)
  number        String       // numer dokumentu np. KP/001/2026
  amount        Decimal      @db.Decimal(12, 2)
  description   String?
  cashShiftId   String?
...
model HotelEvent {
  ...
  description String?  @db.Text
  color       String?  @db.VarChar(20)  // kolor wydarzenia (np. "#FF5733")
  propertyId  String?
  isPublic    Boolean  @default(true)  // czy wydarzenie jest widoczne publicznie
  createdAt   DateTime @default(now())
  updatedAt   DateTime @updatedAt
...
model DocumentNumberingConfig {
  ...
  includeMonth   Boolean  @default(false)  // format numer/miesiąc/rok/literka (np. 1/03/2025/A)
  seriesLetter   String   @default("A")   // litera serii przy includeMonth (np. A, B, C)
  sequenceStart  Int      @default(1)     // od jakiej liczby zaczyna się numeracja; zapis resetuje licznik
...
// Licznik numeracji dokumentów (oddzielny dla każdego typu, roku i opcjonalnie miesiąca)
model DocumentNumberCounter {
  ...
  month          Int      @default(0)  // miesiąc (1-12); 0 = licznik roczny
  lastSequence   Int      @default(0)  // ostatnia użyta sekwencja
  updatedAt      DateTime @updatedAt

  @@unique([documentType, year, month])
  @@index([documentType])
}
...
model InvoiceTemplate {
  ...
  defaultPaymentMethod String?  @default("przelew")  // domyślna forma płatności
  defaultPaymentDays   Int?     @default(14)         // domyślna ilość dni na płatność
  
  // Domyślne miejsce i podpisy
  placeOfIssue     String?  // domyślne miejsce wystawienia
  issuedByName     String?  // domyślna osoba upoważniona do wystawienia
  
  // Ustawienia kolumn tabeli pozycji
  showPkwiu        Boolean  @default(false)  // pokaż kolumnę PKWIU
  showUnit         Boolean  @default(true)   // pokaż kolumnę j.m. (jednostka miary)
  showDiscount     Boolean  @default(false)  // pokaż kolumnę Rabat
  defaultUnit      String?  @default("szt.") // domyślna jednostka miary
...
model UnassignedGastronomyCharge {
  ...
  amount        Decimal  @db.Decimal(12, 2)
  description   String?  @db.Text
  posSystem     String?             // np. "POS-Karczma"
  ...
  assignedToReservation   Reservation? @relation("UnassignedCharges", fields: [assignedToReservationId], references: [id], onDelete: SetNull)
  assignedAt              DateTime?
