...
import { getInvoiceById, updateInvoice, deleteInvoice } from "@/app/actions/finance";
...
export interface InvoiceEditSheetProps {
...
export function InvoiceEditSheet({
  ...
  useEffect(() => {
    ...
    getInvoiceById(invoiceId).then((r) => {
      if (r.success && r.data) {
        const d = r.data;
        setInvoiceDetail({
          number: d.number,
  ...
  const handleSaveInvoiceMainData = async () => {
    if (!invoiceId || !invoiceDetail?.isEditable) return;
  ...
  const handleSaveInvoiceCustomFields = async () => {
    ...
    try {
      const result = await updateInvoice(invoiceId, {
        customFieldValues: Object.keys(customFields).length > 0 ? customFields : null,
      });
      if (result.success) {
  ...
  const handleSaveInvoiceNotes = async () => {
    ...
    setSavingInvoice(true);
    try {
      const result = await updateInvoice(invoiceId, { notes: invoiceNotes.trim() || null });
      if (result.success) {
  ...
  const handleDeleteInvoice = async () => {
    ...
    if (
      !confirm(
        `Czy na pewno usunąć fakturę ${invoiceDetail.number}? Operacja jest nieodwracalna. Numer będzie dostępny do ponownego użycia.`
      )
    )
      return;
          ...
          <div className="mt-4 space-y-4">
            {invoiceDetail.isEditable ? (
              ...
              <div className="text-sm text-muted-foreground space-y-1">
                ...
                <p className="text-xs">
                  Faktury wystawionej / wysłanej do KSeF nie można edytować – użyj korekty faktury.
                </p>
              </div>
            )}
            {invoiceDetail.isEditable && (
